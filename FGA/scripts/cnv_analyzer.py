...
class CNVAnalyzer:
    def __init__(
        ...
        chr_col_name: str = None,
        size_col_name: str = None,
    ):
        ...
        self.chr_col_name = chr_col_name
        self.size_col_name = size_col_name
    ...
    def get_purity_corrected_log2_ratios_cnvs(
        ...
        copy_number_obs = self.get_copy_number_obs(cnvs)
    ...
    def get_filtered_cnvs_by_autosomes(self, cnvs: DataFrame) -> DataFrame:
        ...
        cnvs_filtered = cnvs[cnvs[self.chr_col_name].astype(str).str.isdigit()]
        return cnvs_filtered[
            cnvs_filtered[self.chr_col_name].astype(int).between(1, 22)
        ]

    def get_filtered_cnvs_by_size(self, cnvs: DataFrame) -> DataFrame:
        """Filter CNVs based on minimum size threshold."""
        return cnvs[cnvs[self.size_col_name] >= self.min_size]
    ...
    def calculate_fga_for_sample(self, cnvs: DataFrame) -> float:
        ...
        altered_bp: int = cnvs[self.size_col_name].sum(skipna=True)
    ...
    def compute_fga(self, output_csv) -> None:
        ...
        for filepath in self.data_directory.glob("*.*"):
